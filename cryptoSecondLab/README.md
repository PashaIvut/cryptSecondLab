...
Карта различий визуализирует изменения между исходным и стего-изображением. Каждый пиксель показывает абсолютную разницу значений RGB между двумя изображениями.

##Почему карта однотонная (почти черная):##
- LSB-метод изменяет только младший бит каждого канала RGB
- Максимальная разница между исходным и измененным значением пикселя составляет всего 1 из 255 (0.4%)
- Такие изменения визуально неразличимы и отображаются как почти черный цвет, что подтверждает высокую визуальную незаметность LSB-стеганографии.
